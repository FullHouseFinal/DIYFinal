...
<body style="overflow: visible;">
								...
								<table class="table"
									...
									<thead>
										<tr>
											...
											<th style="WIDTH: 7%;">AUTH</th>
										</tr>
									</thead>
									<tbody>
										<c:forEach items="${companyList}" var="company">
											<tr>
												<td style="WIDTH: 10%;">${company.coName}</td>
												<td style="WIDTH: 10%;">${company.coId}</td>
												<td style="WIDTH: 15%;">${company.coTel}</td>
												<td style="WIDTH: 10%;"><a
													href="/admin/user/companyRead?CoId=${company.coId}">${company.coManagerName}</a></td>
												<td style="WIDTH: 13%;">${company.coManagerDept}</td>
												<td style="WIDTH: 20%;">${company.coManagerEmail}</td>
												<td style="WIDTH: 7%; text-align: center;" id="authority">
													<c:if test="${company.coAuthority == 1}">
												<a href="/admin/user/authorityRemove?coId=${company.coId}">
													<img src="/admin/resources/upload/auth01.png" style="width: 30px;">
													</a>
													</c:if>
													<c:if test="${company.coAuthority == 0}">
												<a href="/admin/user/authority?coId=${company.coId}">
													<img src="/admin/resources/upload/auth02.png" style="width: 30px;">
													</a>
													</c:if>
													</td>
											</tr>
							...
							<div class="col-md-12">
								<nav class="text-center">
									<ul class="pagination" id="navigator">
										<li><a
											href="javascript:pageProc(${companyNavi.currentPage - companyNavi.pagePerGroup},'${companyCondition}','${companyKeyword}')"><i class="fa fa-fast-backward" aria-hidden="true"></i></a></li>
										<li><a
											href="javascript:pageProc(${companyNavi.currentPage - 1},'${companyCondition}','${companyKeyword}')"><i class="fa fa-step-backward" aria-hidden="true"></i></a></li>
										<c:forEach begin="${companyNavi.startPageGroup}"
											end="${companyNavi.endPageGroup}" var="counter">
											<c:if test="${counter == companyNavi.currentPage}"></c:if>
											<li><a
												href="javascript:pageProc(${counter},'${companyCondition}','${companyKeyword}','${companyCondition}')">${counter}</a></li>
											<c:if test="${counter == companyNavi.currentPage}"></c:if>
										</c:forEach>
										<li><a
											href="javascript:pageProc(${companyNavi.currentPage + 1},'${companyCondition}','${companyKeyword}')"><i class="fa fa-step-forward" aria-hidden="true"></i></a></li>
										<li><a
											href="javascript:pageProc(${companyNavi.currentPage + companyNavi.pagePerGroup},'${companyCondition}','${companyKeyword}')"><i class="fa fa-fast-forward" aria-hidden="true"></i></a></li>
									</ul>
								</nav>
							</div>
							<div class="row">
								...
								<div class="col-lg-12" style="margin-top: 20px;">
									<div class="couponarea clearfix">
										<form class="form-inline" style="text-align: center;"
											action="/admin/user/companyList" method="get">
											<div class="sel sel--black-panther"
												...
												<select name="companyCondition" id="companyCondition"
													style="display: none;">
													<option value="coId">ID</option>
													<option value="coManagerEmail">EMAIL</option>
												</select>
											</div>
											<div class="form-group" style="">
												<input type="text" class="form-control"
													name="companyKeyword" placeholder="Enter here..."
	...
	<script>
		function pageProc(currentPage, companyCondition, companyKeyword) {
			location.href = "/admin/user/companyList?currentPage="
					+ currentPage + "&companyCondition=" + companyCondition
